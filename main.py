...
app = Flask('')
@app.route('/')
def home(): return "Multi-AI Bober 3.2 is Running"
...
def callback_query(call):
    ...
    if call.data.startswith("set_mode_"):
        ...
        if mode == "restore":
            bot.edit_message_text("Выбрана реставрация. Интенсивность лица:", chat_id, call.message.message_id, reply_markup=get_restore_keyboard())
        else:
            bot.edit_message_text(f"Режим {mode} готов! Присылай фото. 📸", chat_id, call.message.message_id)
    
    elif call.data.startswith("set_fid_"):
        user_data[chat_id]["fid"] = float(call.data.replace("set_fid_", ""))
        bot.edit_message_text(f"Настройки сохранены. Жду фото! 📸", chat_id, call.message.message_id)
...
def handle_photo(message):
    ...
    msg = bot.reply_to(message, f"⏳ Запускаю {settings['mode']}... Почти готово.")
    ...
    try:
        ...
        with Image.open(input_path) as img:
            img = img.convert("RGB")
            img.thumbnail((1000, 1000)) # Чуть меньше для скорости
            img.save(input_path, "JPEG", quality=85)

        client = Client(MODELS[settings['mode']], token=HF_TOKEN.strip())
        
        # --- ГИБКАЯ ЛОГИКА ВЫЗОВА ---
        if settings['mode'] == "restore":
            # Для CodeFormer используем позиционные аргументы
            ...
            # Для AnimeGANv2 указываем версию стиля явно через индекс или параметры
        ...
        with open(output_path, 'rb') as f:
            bot.send_document(chat_id, f, caption=f"✨ Твой {settings['mode']}-результат готов!")
        
        bot.delete_message(chat_id, msg.message_id)

    except Exception as e:
        print(f"Error: {e}", flush=True)
        # Если не сработал обычный вызов, пробуем запасной вариант для Аниме
        bot.edit_message_text(f"❌ Ошибка нейросети. Попробуй другое фото или смени режим.\nПодсказка: {str(e)[:50]}...", chat_id, msg.message_id)
    
    finally:
        if os.path.exists(input_path): os.remove(input_path)

if __name__ == "__main__":
    print("--- Бобёр 3.2 запущен (Anime Fix) ---", flush=True)
    bot.remove_webhook()
